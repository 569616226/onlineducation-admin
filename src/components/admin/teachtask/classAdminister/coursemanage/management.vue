<template>
	<div v-loading="loading">
		<div class="admin_title">
			<i class="el-icon-erp-icon-keyboard-backspace el-icon--left veal_middle cursor" @click="resetForm"></i> {{$route.meta.title}}（{{data.name}}）
		</div>
		<el-row class="management_first margin-top-sm">
			<el-col :span="5">
				<el-button type="primary" @click="AddPart">+新建节</el-button>
			</el-col>
			<el-col :span="10" :offset="9" class="management_second">
				<span class="margin-right-sm">节总数：</span><span class="margin-right-sm green">{{data.sections.length}}</span><span class="margin-right-sm">学习时长：</span><span class="red">{{videotimes}}</span><span class="margin-left-sm">播放次数：</span><span class="blue margin-right-sm">{{ playtimes }}</span>
			</el-col>

			<el-col :span="24" class="margin-top-sm management_first_class" v-for="(section,index) in data.sections" :key="section.id" :value="section.id">
				<i class="el-icon-erp-icon-ic_ondemand_video"></i><span class="margin-left-sm cursor" @click="videoshow(section)">{{section.name}}（<span>{{section.video.duration}}</span>）</span>
		...
		<!--新建节-->
		<el-dialog v-loading="loadingsec" title="新建节" :visible.sync="dialogTableVisible" @close="reset()">
			<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
				...
				<el-form-item>
					<data-tables ref="NewTable" :data="videos" :pagination-def="paginationDef" :table-props="tableProps" :show-action-bar="false" :custom-filters="customFilters" @row-click="handleCurrentChange">
						...
						<el-table-column prop="name" label="视频名称" key="name" sortable="custom">
							<template slot-scope="scope">
								<div :class="scope.row.checked == 1 ? 'blue' : ''">{{scope.row.name}}</div>
							</template>
						</el-table-column>
			...
			<div slot="footer" class="dialog-footer">
				<el-button type="primary" :disabled="loadingsec" @click="submitForm('ruleForm')">提 交</el-button>
		...
		<!--编辑节信息-->
		<el-dialog v-loading="loadingsec" title="编辑节信息" :visible.sync="dialogeditTableVisible" @close="reset()">
			<el-form :model="ruleFormedit" :rules="rules" ref="ruleFormedit" label-width="120px" class="demo-ruleForm">
				...
				<el-form-item>
					<data-tables :data="videos" :pagination-def="paginationDef" :table-props="tableProps" :show-action-bar="false" :custom-filters="customFilters" @row-click="edithandleCurrentChange">
						<el-row slot="custom-tool-bar" style="margin-bottom: 10px" class="class_el_row">
							<el-col :span="24">
								<el-input v-model="customFilters[0].vals" prefix-icon="el-icon-erp-icon-ic_search" placeholder="搜索课程" class="class_input_width">
								</el-input>
			...
			<div slot="footer" class="dialog-footer">
				<el-button type="primary" :disabled="loadingsec" @click="submitFormedit('ruleFormedit')">提 交</el-button>
	...
	export default {
		...
		methods: {
			AddPart() {
				...
				if(this.data.type == '访谈课程') {
					if(this.data.sections.length == 1) {
						this.$message.warning('访谈课程只能新建一个')
					} else {
			...
			editvideo(section) {
				...
				this.videos.forEach(e =>{
					if(this.ruleFormedit.video_name == e.name){
						e.checked = 1
					}else{
						e.checked = 2
					}
			...
			reset() {
				this.ruleForm.name = '';
				this.ruleForm.video_name = '';
				this.customFilters[0].vals = '';
			},
			//新建章节提交
			submitForm(formName) {
				let that = this
			...
			videoshow(vid) {
				...
				this.videoshowurl = vid.video.url;
		...
		watch: {
			// 监听data，当有新增章节或者是删除章节的时候重新计算播放次数和学习时长
			data: {
				handler(val) {
					this.timesarray = [];
					this.playtimes = 0;
					for(let i = 0; i < val.sections.length; i++) {
						this.timesarray.push(val.sections[i].video.duration);
						this.playtimes += parseInt(val.sections[i].play_times);
					};
		...
		mounted: function() {
			...
			for(let i = 0; i < this.data.sections.length; i++) {
				this.timesarray.push(this.data.sections[i].video.duration);
				this.playtimes += parseInt(this.data.sections[i].play_times);
			};
			this.videotimes = this.addTimes(this.timesarray);
			this.getAll()
		}
